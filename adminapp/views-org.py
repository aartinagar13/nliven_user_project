    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            if request.user.is_authenticated:
                ...
                print('user details:',user)
                search_term = request.GET.get('searchInput')
                if user.role == Role.objects.filter(name='Manager'):
                    # Manager can see all data
                    print('if block')
                ...
                # elif user.role == Role.objects.filter(name='user'):
                else:
                    print('elif block')
                    print(request.user)
                    # Profile can see their own data
...
class Search_userlistView(View):
    template_name = 'admin/user_list.html'
    search_query =''
...
def delete_user(request, myid):
    if request.user.is_superuser:
        if Profile.objects.filter(id=myid).exists():
            user =Profile.objects.get(id=myid)
            user.delete()
...
def store(request):
  try: 
    ...
    if request.user.is_superuser:
        ...
        print('confirm_password',repassword)
        role= request.POST.get('role')
        ...
        if request.POST.get('user_id'):
            ...
            user.email=email
            user.image=image 
            if not firstname or not lastname or not email or not password or not repassword:
            ...
            print('#Profile =  get_user_model()')
            print(username,firstname, lastname, email, password,image, gender)  
            if not firstname or not lastname or not email or not password or not repassword:
                messages.error(request, "All fields are required.")
                return redirect('add')
            ...
            if Profile.objects.filter(email=email).exclude(id=request.POST.get('user_id', 0)).exists():
                messages.error(request, "Email is already in use.")
                return redirect('add')
            user = Profile.objects.create_user(username=username,first_name=firstname,last_name=lastname,email=email,password=password,role_id=role,gender_id=gender,image=image)
            user.save()
        ...
        if  Profile.objects.filter(username=username).exists():
            messages.success(request,"Already exsits user with this username")
            return redirect('users_list')
